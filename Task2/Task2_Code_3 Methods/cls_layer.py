    ...
    def __init__(self, config):
        ...
        for layer in self.classifier:
            if isinstance(layer, torch.nn.Linear):
                next_tanh = False
                try:
                    idx = list(self.classifier.children()).index(layer)
                    if idx < len(self.classifier)-1 and isinstance(self.classifier[idx+1], torch.nn.Tanh):
                        next_tanh = True
                except ValueError:
                    pass
                ...
                torch.nn.init.kaiming_normal_(
                    layer.weight, 
                    mode='fan_in',
                    nonlinearity='linear' if next_tanh else 'selu'
                )
...
def train(args):
    ...
    # Add learing rate warmup cosine schedule 
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer,
        T_0=50,           # Number of iterations for first restart
...
def get_args():
    ...
    parser.add_argument("--attention_probs_dropout_prob", type=float, default=0.2, help="Dropout probability for attention probabilities")
