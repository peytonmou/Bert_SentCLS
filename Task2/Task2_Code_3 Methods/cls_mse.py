...
class BertSentClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):   
        ...
        return logits      # raw logits for MSE, this updated for Task2
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            target = F.one_hot(b_labels, num_classes=num_labels).float().to(device)
            loss_fn = torch.nn.MSELoss()                  # updated for Task2
